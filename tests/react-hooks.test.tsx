...
import { AppSyncEventsClient, Channel } from '../lib/appsync-events-client';
...
describe('React Hooks', () => {
  ...
  describe('useChannel', () => {
    ...
    it('should unsubscribe when unmounted', async () => {
      ...
      await act(async () => {
        const { unmount } = renderHook(() => 
          useChannel(mockClient, 'test/channel', vi.fn())
        );
        unmountFn = unmount;
      });
    ...
    it('should handle callback changes', async () => {
      ...
      expect(initialCallback).toHaveBeenCalledWith({ message: 'Hello, world!' });
      ...
      expect(updatedCallback).toHaveBeenCalledWith({ message: 'Updated message!' });
    });
  });
});